...
const SlotRegistrationModal = ({ mission, eventId, isEventPast, onClose }: ISlotRegistrationModalProps) => {
    ...
    return (
        <dialog id={`slot_registration_modal_${mission.id}`} className="modal">
            ...
            <div className="modal-box max-w-none w-full h-full rounded-none md:w-110 md:h-fit md:rounded-xl bg-[#e6dabb] dark:bg-[#1e2433] flex flex-col gap-4 justify-between">
                ...
                <button onClick={onClose} className="btn btn-sm self-end bg-transparent text-lg btn-ghost text-[#104e64] dark:text-[#e6dabb] md:hidden">
                    Fermer
                </button>
            </div>
